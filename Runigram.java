...
public class Runigram {
	...
	public static Color[][] grayScaled(Color[][] image) {
		Color[][] greyScaledC = new Color[image.length][image[0].length];

		for (int i = 0; i < image.length; i++) {
			for (int j = 0; j < image[i].length; j++) {
				greyScaledC[i][j] = luminance(image[i][j]);
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				scaledArr[i][j] = image[(i*image.length/(height))][j*image[0].length/width];
	...
	public static Color blend(Color c1, Color c2, double alpha) {
		//int r = alpha*   (double)c1.getRed() + (1-alpha)*c2.getRed();
